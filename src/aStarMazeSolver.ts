...
export class AStarMazeSolver{
    ...
    constructor(start: MazeCell,finish: MazeCell,mazeGrid: MazeCell[][], mazeSizes: number[]) {
      this.start = start;      
      this.finish = finish;
    ...
    setup() {
      ...
      this.start.setFScore(this.getManhattanDistance(this.start, this.finish) + this.start.getGScore());
    }
    ...
    solveStep() {
      ...
      for(let i = 1; i<this.neighbourCounter+1; i++) {
        ...
        previousPath.setGreenColour(false);
      }
      this.neighbourCounter = 0;
      ...
      if(this.paths.length > 0 && this.solved == false) {
        ...
        if (current === this.finish) {
          ...
          current.fillCellColour(current.getColumnNumber(),current.getRowNumber(),this.mazeSize,this.mazeColumns,this.mazeRows, greyColour,false)
          ...
          if(neighbourCells){
            for(let neighbour of neighbourCells) {
              ...
              if(new_gscore < neighbour.getGScore()){
                ...
                neighbour.setFScore(new_gscore + this.getManhattanDistance(neighbour,this.finish));
                ...
                if(!this.paths.includes(neighbour)) {
                  ...
                  neighbour.fillCellColour(neighbour.getColumnNumber(), neighbour.getRowNumber(), this.mazeSize, this.mazeColumns, this.mazeRows, greenColour,false)
                  neighbour.setGreenColour(true);
                }
              }
            }
          }
