...
export class MazeCell {
    ...
    checkNeighbours() {
        ...
        let bottom =
            row !== grid.length - 1
                ? grid[row + 1][this.columnNumber]
                : undefined;
        let left = col !== 0 ? grid[row][col - 1] : undefined;
        let right =
            col !== grid[row].length - 1 ? grid[row][col + 1] : undefined;
    ...
    fillBackTrackLine(
        ...
        // Corner: bottom to right/right to bottom
            ...
            const horizontalLine = new THREE.Mesh(
                new THREE.BoxGeometry(
                    (this.parentGridSize / columns) * 0.5,
                    (this.parentGridSize / rows) * 0.2,
                    depth
                ),
                new THREE.MeshLambertMaterial({ color: colour })
            ...
            const horizontalLineAdjustmentX =
                3 * (parentGridSize / (4 * columns));
