...
export class MazeCell {
  ...
  private activeSquareColour: boolean;
  ...
  constructor(
    ...
    this.cellObjects = {
      ...
      greySquare: null,
      yellowSquare: null,
      backTrackingLineHorizontal: null,
      backTrackingLineVertical: null,
    };
  ...
  checkNeighbours() {
    ...
    let bottom =
      row !== grid.length - 1 ? grid[row + 1][this.columnNumber] : undefined;
  ...
  drawRightWall(
    ...
    wall.position.x +=
      (x * parentGridSize) / columns + parentGridSize / columns;
    wall.position.y -= (y * parentGridSize) / rows;
  }
  ...
  fillBackTrackLine(
    ...
    if (lineType === 1) {
      const verticalLine = new THREE.Mesh(
        new THREE.BoxGeometry(
          (this.parentGridSize / columns) * 0.2,
          (this.parentGridSize / rows) * 1,
          depth
        ),
      ...
      verticalLine.position.set(
        initialX + lineAdjustmentX,
        initialY - lineAdjustmentY,
        zpos
      );
      ...
      // Add line to cell objects
      this.cellObjects.backTrackingLineVertical = verticalLine;
    }
    // Horizontal Line
    else if (lineType === 2) {
      const horizontalLine = new THREE.Mesh(
        new THREE.BoxGeometry(
          (this.parentGridSize / columns) * 1,
          (this.parentGridSize / rows) * 0.2,
          depth
        ),
        new THREE.MeshLambertMaterial({ color: colour })
      );
      ...
      // Add line to cell objects
      this.cellObjects.backTrackingLineHorizontal = horizontalLine;
    }
    // Corner: bottom to right/right to bottom
      ...
      const horizontalLine = new THREE.Mesh(
        new THREE.BoxGeometry(
          (this.parentGridSize / columns) * 0.7,
          (this.parentGridSize / rows) * 0.2,
          depth
        ),
        new THREE.MeshLambertMaterial({ color: colour })
      ...
      horizontalLine.position.set(
        initialX + horizontalLineAdjustmentX,
        initialY - horizontalLineAdjustmentY,
        zpos
      );
      verticalLine.position.set(
        initialX + verticalLineAdjustmentX,
        initialY - verticalLineAdjustmentY,
        zpos
      );
  ...
  fillCellColour(
    ...
    if (backtrack) {
      ...
      const square = new THREE.Mesh(
        new THREE.BoxGeometry(
          this.parentGridSize / columms + 0.7,
          this.parentGridSize / rows + 0.7,
          1
        ),
        new THREE.MeshLambertMaterial({ color: colour })
      ...
      // Store square object so it can be deleted/removed from scene later -- active square still tied to greenColour
      if (colour === Colours.greenColour) {
        this.cellObjects.activeSquare = square;
      ...
      square.position.set(
        initialX + squareAdjustmentX,
        initialY - squareAdjustmentY,
        3
      );
  ...
  // Probably should extend functionality of this function, but its just not needed, its constricted to the green and grey colour
  // BIGG TODO
  removeCellColour(cell: MazeCell, colour: number) {
    if (colour === Colours.greenColour) {
      let obj = cell.cellObjects.activeSquare;
      this.removeObject3D(obj);
    } else if (colour === Colours.greyColour) {
  ...
  removeAllCellObjects() {
    const topWall = this.cellObjects.topWall;
    const leftWall = this.cellObjects.leftWall;
    const rightWall = this.cellObjects.rightWall;
    const bottomWall = this.cellObjects.bottomWall;
    const activeSquare = this.cellObjects.activeSquare;
    const greySquare = this.cellObjects.greySquare;
    const yellowSquare = this.cellObjects.yellowSquare;
    const backTrackingLineHorizontal =
      this.cellObjects.backTrackingLineHorizontal;
    const backTrackingLineVertical = this.cellObjects.backTrackingLineVertical;

    this.removeObject3D(topWall);
    this.removeObject3D(leftWall);
    this.removeObject3D(rightWall);
    this.removeObject3D(bottomWall);
    this.removeObject3D(activeSquare);
    this.removeObject3D(greySquare);
    this.removeObject3D(yellowSquare);
    this.removeObject3D(backTrackingLineHorizontal);
    this.removeObject3D(backTrackingLineVertical);
  }
  ...
  setActiveSquareColour(value: boolean) {
    this.activeSquareColour = value;
  }
